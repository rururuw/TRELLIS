    ...
    def sample(
        ...
        **kwargs
        ...
        for t, t_prev in tqdm(t_pairs, desc="Sampling", disable=not verbose):
            ...
            v_target = v_guided + (v_pos - v_neg) * slider_scale
        ...
        # manipulating latent using denoised input here using the last t_pair adapted from ConceptSlider. 11/22 didn't really work.
        # for t, t_prev in tqdm(t_pairs[step_to:], desc="Sampling", disable=not verbose):
        #     # v_neutral = self._inference_model(model, sample, t, neutral_cond)
        #     v_pos = self._inference_model(model, sample, t, cond)
        #     v_neg = self._inference_model(model, sample, t, neg_cond)
        #     v_target = v_neg + (v_pos - v_neg) * cfg_strength
        #     pred_x_0, pred_eps = self._v_to_xstart_eps(x_t=sample, t=t, v=v_target)
