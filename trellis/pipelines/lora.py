...
#     "Transformer2DModel",  # どうやらこっちの方らしい？ # attn1, 2. translate to English correctly: apparently this is the one. # attn1, 2
    "Attention"
]
...
class LoRAModule(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("alpha", torch.tensor(alpha))  # 定数として扱える. translate to English: register as a constant.
...
class LoRANetwork(nn.Module):
    def __init__(
        ...
        # assertion 名前の被りがないか確認しているようだ. translate to English: check for duplicate names.
        lora_names = set()
        ...
        for name, module in root_module.named_modules():
            if train_method == "noxattn" or train_method == "noxattn-hspace" or train_method == "noxattn-hspace-last":  # Cross Attention と Time Embed 以外学習
                if "attn2" in name or "time_embed" in name:
                    continue
            elif train_method == "innoxattn":  # Cross Attention 以外学習. translate to English correctly: train only non-cross attention layers.
                if "attn2" in name:
                    continue
            elif train_method == "selfattn":  # Self Attention のみ学習. translate to English: train only self attention layers.
                if "attn1" not in name:
                    continue
            elif train_method == "xattn" or train_method == "xattn-strict":  # Cross Attention のみ学習. translate to English: train only cross attention layers.
                if "attn2" not in name:
                    continue
            elif train_method == "full":  # 全部学習. translate to English: train all layers.
                pass
            else:
